...
class particle {
    ...
    // method to draw invidual particle
    draw() {
        ctx.beginPath();
        ctx.arc(this.x, this.y, this.size, 0, Math.PI * 2, false);
        ctx.fillStyle = 'rgba(102, 211, 250, 0.605)';
        ctx.fill();
    }
...
function init(){
    ...
    for (let i = 0; i < numberOfParticles*2; i++){
        ...
        let color = 'rgba(102, 211, 250, 0.605)';
    ...
    for(let a = 0; a < particlesArray.length; a++){
        for (let b = a; b < particlesArray.length; b++) {
            ...
            if (distance < (canvas.width/7) * (canvas.height/7)) {
                opacityValue = 0.605 - (distance/1000000);
                ctx.strokeStyle = 'rgba(102, 211, 250, ' + opacityValue + ')';
                ctx.lineWidth = 1;
                ctx.beginPath();
